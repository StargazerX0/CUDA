...
__global__ void rabinKarpKernelOptimized(
    ...
    if (sig_length > sample_length) {
        d_match_scores[global_thread_id] = 0.0;
        return;
    }
    ...
    for (int i = 0; i <= sample_length - sig_length; ++i) {
        ...
        if (window_hash == sig_hash) {
            ...
            for (int j = 0; j < sig_length; j++) {
                ...
                current_confidence += static_cast<double>(phred);
            }
            if (match) {
                match_found = true;
    ...
    if (match_found && match_count > 0) {
        d_match_scores[global_thread_id] = confidence_sum / static_cast<double>(match_count);
    } else {
        d_match_scores[global_thread_id] = 0.0; // No match
    }
}
